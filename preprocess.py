...
def preprocess(df:pd.DataFrame): 
    ...
    # property type, neighbourhood_cleansed, name
    
    # dummies = pd.get_dummies(df["property_type"], prefix="prop")
    # df = df.join(dummies) 
    
    # dummies = pd.get_dummies(df["neighbourhood_cleansed"])
    # df = df.join(dummies) 
