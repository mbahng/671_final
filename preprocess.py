...
# Defining a list of relevant amenities to look for in the dataset.
...
def contains(x): 
    # Check if the string 'shared' is in the input text.
    return int("shared" in str(x).lower())

def numBath(x): 
    # Extract the number of bathrooms from the text, handling 'half' bathrooms.
    if "half" in x.lower(): 
        return 0.5 
    else: 
...
def numAdjectives(text): 
    # Count the number of adjectives in the input text.
...
def sentiment(text):
    # Analyze the sentiment of the input text and return polarity scores.
    sia = SentimentIntensityAnalyzer()
    return sia.polarity_scores(text)

def numBedroom(x): 
    # Extract the number of bedrooms from the text.
    if "bedroom" not in x: 
        return 1 
    else: 
...
def rating(x): 
    # Extract the rating from the text, with a default value if not found.
...
def preprocess(df:pd.DataFrame): 
    ...
    # Convert date columns to datetime format and calculate the number of days hosted.
    ...
    # Convert categorical binary columns to numerical format.
    ...
    # Apply custom functions to extract or transform features.
    ...
    # Perform one-hot encoding on the presence of each host verification
    ...
    # Iterate over each element in relevant_amenities (extracted to be the representative amenities for each class) and store its presence in new columns
    ...
    # Take 'name' and extract 'num_bedroom' (default=1) and 'rating' (default=4.74 known as mean rating of Airbnb)
    ...
    # also the word count to see how descriptive the description is
    df["wordCount"] = df["description"].apply(wordCount)
    df["numAdjectives"] = df["description"].apply(numAdjectives)
    
    # extract sentiment scores from the description as well
    ...
    # one hot encode neighbourhood group cleansed
    one_hot = pd.get_dummies(df['neighbourhood_group_cleansed'])
    df = df.join(one_hot)
    
    # remove irrelevant/redundant features and features that have too many classes/too difficult to extract features from 
